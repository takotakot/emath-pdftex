...
\section*{はじめに}
\hakosyokika
センター試験では $1024=2^{\protect\Hako<2>(0\zw)}$ のように，
...
左右あわせて約 \verb/1\zw/（全角1文字分），
正しくは可変長で 
\begin{quote}
\verb/1\zw plus .5\zw minus .05\zw/
\end{quote}
の余白を付けています．
...
\begin{itemize}\itemindent2\zw\item[【選択肢】]
\renewcommand{\labelenumi}{\maru{\theenumi}}%
\begin{edaenumerate}<4>
\ttfamily\edaitemindent2\zw%
\edaitem{+}
\edaitem{-}
\edaitem{$\times$}
\edaitem{$\div$}
...
\renewcommand{\labelenumii}{注\arabic{enumii}．\ }
      \begin{enumerate}
        \item その他の文字はそのまま出力されますから，\\
              \hspace{2\zw}/(1)/ と指定すれば，\\
              \hspace{4\zw}(1),(2),(3),(4),(5),.....\\
                   と，ナンバリングされます．
        \item デフォルトは /ア/ ですが，
              \begin{jquote}
                \verb/\hakomozisyu{い}/
...
$f(x)=x^n$の導関数は
\[f'(x)=\maskHako(4\zw)[2\zh][1\zh]{nx^{n-1}}\]
である。
\end{showEx}
...
\begin{showEx}(.8,.8){解答を赤で埋め込む}
  ${(x^2)}'=\maskHako(4\zw)[2\zh][1\zh]{\color{red}2x}$
\end{showEx}
...
次の\karaHako を補え。
\begin{align*}
  {(x^2)}'&=\maskHako(4\zw)[2\zh][1\zh]{2x}\\
  {(x^3)}'&=\maskHako(4\zw)[2\zh][1\zh]{3x^2}
\end{align*}
\end{showEx}
